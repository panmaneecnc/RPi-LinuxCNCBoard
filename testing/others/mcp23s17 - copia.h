...
//class MCP {
  //public:
    void configMCP23s17(uint8_t);                            // Constructor to instantiate a discrete IC as an object, argument is the address (0-7) of the MCP23S17
    ...
    void pinMode_bit(uint8_t, uint8_t);          // Sets the mode (input or output) of a single I/O pin
    void pinMode_word(unsigned int);              // Sets the mode (input or output) of all I/O pins at once 
    void pullupMode_bit(uint8_t, uint8_t);       // Selects internal 100k input pull-up of a single I/O pin
    void pullupMode_word(unsigned int);           // Selects internal 100k input pull-up of all I/O pins at once
    void inputInvert_bit(uint8_t, uint8_t);      // Selects input state inversion of a single I/O pin (writing 1 turns on inversion) 
    void inputInvert_word(unsigned int);          // Selects input state inversion of all I/O pins at once (writing a 1 turns on inversion)
    void digitalWrite_bit(uint8_t, uint8_t);     // Sets an individual output pin HIGH or LOW
    void digitalWrite_word(unsigned int);         // Sets all output pins at once. If some pins are configured as input, those bits will be ignored on write
    uint8_t digitalRead_bit(uint8_t);            // Reads an individual input pin
    uint8_t byteRead(uint8_t);               // Reads an individual register and returns the byte. Argument is the register address
    unsigned int digitalRead_word(void);          // Reads all input  pins at once. Be sure it ignore the value of pins configured as output!
  //private:
